...
ofColor ofApp::getSlitPixelColor( int x, int y ){
	//Calculate the distance from (x,y) to the current mouse position mouseX, mouseY
	...
	//We need it for getting a "smooth result" by interpolating colors later
...
void ofApp::setup(){
	...
	video.loadMovie("break.mov");
	
	//Play video with 1/4 of its normal speed for better seeing slit-scan effect
	video.setSpeed(0.25);
...
void ofApp::update(){
	...
	//Do compputing only if a new frame was obtained
	if(video.isFrameNew()){
		//Push the new frames to the beggining of the frame list
		frames.push_front(video.getPixelsRef());

		//If number of buffered frames > N, then pop the oldest frame
		if(frames.size() > N){
			frames.pop_back();
		}
	}

	//It is possible that video player did not finish decoding the first frame at first testApp::update() calling, so we need check, if there are frames
	if ( !frames.empty() ) {
		//Now constructing the output image in imagePixels 
		
		//If imagePixels is not initialized yet, then initialize it by copying from any frame.
		//This is simplest way to create a pixel array of the same size and type
...
void ofApp::draw(){
	/*
	//Getting reference for the pixel array
	ofPixels &pixels = video.getPixelsRef();

	//Define variables w, h equal to frame width and length, respectively
