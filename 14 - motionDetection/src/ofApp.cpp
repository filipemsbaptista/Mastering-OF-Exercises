...
void ofApp::update(){
	...
	if(video.isFrameNew()){
		...
		if(grayImagePrev.bAllocated){
			...
			//Amplify the difference to obtain better visbility of motion
			//We'll do it by multiplication. But first, convert diff to float image
		...
		for(int y = 0; y < h; y++){
			for(int x = 0; x < w; x++){
				...
				if ( value >= 0.9 ) {
					...
					//Note, this is slow function, we use it here just for simplicity
				}
			}
		}
