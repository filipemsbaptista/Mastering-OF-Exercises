...
float Rad = 25;				//Raduius of circle
float circleStep = 3;		//Step size of circle motion
...
void ofApp::addRandomCircle( ofMesh &mesh ){
	...
	if ( base >= 0 ) {			//Check if it is not the first step and we really need to add the triangles
		for (int i=0; i<circleN; i++) {
			int a = base + i;
